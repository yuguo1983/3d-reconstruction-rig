...
- [x] **稠密重建**:`colmap patch_match_stereo` + `stereo_fusion` 生成稠密点云 → `dense_recon.sh`
- [x] **网格重建**:`colmap poisson_mesher` → 输出 `dense/meshed-poisson.ply`
- [x] **纹理贴图**:Python 实现 `texturize.py`(基础版), 或装 mvs-texturing(高级版)
- [x] **格式转换**:`.obj` ↔ `.glb` / `.stl`(用 trimesh)
- [ ] **真实硬件冒烟测试**:6 路 UVC 接入,跑通 §3 全流程,目视检视 `sparse_test/0/` 和 `dense/model.obj`
...
# 稠密重建 + Poisson 网格 (CPU, patch_match 慢)
bash dense_recon.sh
# 输出: dense/fused.ply  +  dense/meshed-poisson.ply

# 贴纹理 → 标准 .obj (引用 6 张相机原图)
python texturize.py --mesh dense/meshed-poisson.ply --sparse dense
# 输出: dense/model.obj  +  dense/model.mtl
...
```
┌─────────────────┐
│  rig.json       │  ← 6 路相机的 index/分辨率/曝光
└────────┬────────┘
         │
         ▼
┌─────────────────────────────────────────────┐
│ 1. 抓帧                                    │
│  rig_ui.py  (or  rig_capture.py)           │
│  输入:  rig.json                           │
│  输出:  captures/<session>/<gid>_<cam>.jpg │
└────────┬────────────────────────────────────┘
         │   6 cams × N groups
         ▼
┌─────────────────────────────────────────────┐
│ 2. 重组目录                                │
│  reorganize_captures.py                    │
...
│ 3. 联合标定 (ChArUco)                       │
│  calibrate_rig.py                          │
│  输出:  calib/rig_calib.json               │
│         calib/cameras.txt  (COLMAP 格式)   │
│         calib/rig_layout.png  (3D 俯视)     │
└────────┬────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────┐
│ 4. 标定 → COLMAP rig schema                │
│  rig_to_colmap_config.py                   │
│  输出:  rig_config.json                    │
└────────┬────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────┐
│ 5. 自检                                    │
│  validate_rig_config.py                    │
│  模仿 COLMAP 端 ReadRigConfig 全部检查     │
└────────┬────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────┐
...
│  输出:  sparse_test/0/{cameras,images,     │
│                       points3D}.txt        │
└────────┬────────────────────────────────────┘
         │
         ▼  (可选, 要拿 3D 模型才跑)
...
│ 8. 贴纹理                                  │
│  texturize.py                              │
│  输出:  dense/model.obj + model.mtl        │
│         (引用 dense/images/*.jpg 6 张图)    │
└────────┬────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────┐
│ 9. 格式转换 (可选)                          │
│  trimesh: .obj → .glb / .stl / .ply        │
│  MeshLab / Blender: 可视化、编辑、导出      │
└─────────────────────────────────────────────┘
```
...
```bash
# 一键跑完全部 (1-9 步), 约 15-20 分钟
bash run_pipeline.sh
```

期望看到:
- ✓ 6 路 ChArUco 全部 48/48 角点
- ✓ `database.db` ≈ 30 MB
- ✓ `sparse_test/0/images.txt` 非空
- ✓ `dense/fused.ply` 几 MB
- ✓ `dense/meshed-poisson.ply` 几 MB
- ✓ `dense/model.obj` + `model.mtl` 几十 KB
...
```bash
# [A] 确认 6 路都能开
python probe_cameras.py --max-index 8
# → ./probe/probe_index_*.jpg, 左上角烧了 index 编号
# → 在每个相机旁贴上 index 标签

# [B] 把 index 填进 rig.json
#  {"index": 0, "name": "cam0_north", ...}
#  {"index": 3, "name": "cam3_south", ...}
#  ...

# [C] 打开 UI 看 6 路实时预览 + 帧率
python rig_ui.py
# 期望: 6 路都 28-30 fps, 任何一路掉到 20 以下 → USB 带宽问题

# [D] 打印 ChArUco 板
# 已有 charuco_9x7_20mm.png, 18cm × 14cm, A3 哑光相纸
# 必须 [实际尺寸] 打印, 拿尺子量一下确认 20mm

# [E] 抓 30+ 组 ChArUco 多角度图 (UI 上 [📸 抓 N 组])
#   - 板要倾斜旋转, 6 路同时能看到
#   - 板要占视野中央 60% 以上

# [F] 标定
python calibrate_rig.py --captures ./captures --out ./calib
# 期望: 6 台相机全部标定成功, RMS < 1px
# 不好? 见 README §6 已知问题
...
bash dense_recon.sh
python texturize.py --mesh dense/meshed-poisson.ply --sparse dense
# 打开 dense/model.obj 看结果
```

### 10.3 想在 GPU 服务器上跑(加速 patch_match)
...
耗时从 30-60 分钟降到 **3-5 分钟**。Windows 无 CUDA 时用 `-1` 走 CPU。

### 10.4 想换 mvs-texturing 出更高质量纹理
...
# Windows: 需要 Visual Studio 2019+ + Qt5

# 2. 用它做纹理 (替换 texturize.py)
cd /path/to/3dremodule
/path/to/mvs-texturing/build/Release/mvs-texturing     --dense-point-cloud=dense/fused.ply     --images-dir=dense/images/     --output-dir=dense/textured/
# 输出: dense/textured/model.obj + textured/*.jpg
```
